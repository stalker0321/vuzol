...
def _validate_proxy_destination(host: str, port: int | None) -> None:
    if _is_ip_literal(host):
        raise ValueError(f"IP literal egress destinations are prohibited: {host}")
    # Only 443 allowed for provider targets in this policy
    if port not in (None, 443):
        raise ValueError(f"provider egress must use port 443 (got {port})")
    # Additional hostname sanity (beyond EgressDestination). Note IPs already rejected above.
    forbidden = {
        "localhost",
...
def compile_proxy_allowlist(
    ...
    for dest in policy.destinations:
        ...
        _validate_proxy_destination(host, port)
        canon_host = _normalize_hostname(host)
        targets.append(
    ...
    for raw_host in provider_api_hosts:
        if not raw_host:
            continue
        # provider hosts are plain hostnames, assume 443. Use safe extract for v6 etc.
        host = _extract_host(raw_host)
        _validate_proxy_destination(host, 443)
        canon_host = _normalize_hostname(host)
        targets.append(
            AllowedConnectTarget(
                hostname=canon_host,
