...
def render_worker_prompt(capsule: WorkerTaskCapsule, *, repository_id: str) -> str:
    return (
        ...
        "Vuzol has already prepared and verified the isolated worktree. /workspace is writable "
        "for ordinary working-tree files within the allowed paths. You must actually inspect "
        "and edit those files to complete the requested implementation. You may use the "
        "available shell-backed repository tools to read files, search repository contents, "
        "create and edit ordinary files, and inspect the result using only permitted tools.\n"
        "Provider-specific execution policy determines whether separate read-only Git commands "
        "or local check commands are available. Use them only when that policy explicitly "
        "allows them. Never stage, commit, reset, clean, push, or write .git. Any local check is "
        "only an agent-attempted check, not a required or trusted Vuzol gate. Do not install "
        "packages, synchronize dependencies, access the network, manage services or processes, "
        "manage Docker or containers, access paths outside /workspace, or modify files outside "
        "the explicit allowed paths. Do not touch another VPS project. Unavailable Git or local "
        "checks do not prevent repository inspection or edits; Vuzol independently runs trusted "
        "gates and creates the commit after you exit.\n"
        ...
        "what was done without quoting code, agent_checks with one honest bounded entry for each "
        "relevant local check (including checks not run), or an empty list when none are relevant, "
        "limitations, failure classification, and provider usage "
