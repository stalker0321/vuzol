...
def render_worker_prompt(capsule: WorkerTaskCapsule, *, repository_id: str) -> str:
    return (
        ...
        "create and edit ordinary files, and inspect the result of your edits without Git.\n"
        "Do not invoke any Git command or read or write .git. Do not run required gates or "
        "tests, install packages, synchronize dependencies, access the network, manage services "
        "or processes, manage Docker or containers, access paths outside /workspace, or modify "
        "files outside the explicit allowed paths. Do not touch another VPS project. The lack "
        "of permission to run Git or tests does not prevent repository inspection or edits; "
        "Vuzol runs gates and creates the commit after you exit.\n"
